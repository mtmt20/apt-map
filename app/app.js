    ...
    function render() {
      ...
      $("#detailView").innerHTML = `
        ...
        <div class="section"><h4>배정 학군 <span class="r muted">${esc(state.meta.zone_note || "초등 통학구역 기준")}</span></h4>
          ...
          <div class="mids">${c.school.high.general.map((h) => `<span class="tag">${esc(h.name)} <span class="muted">${h.dist}m${h.public === "사립" ? " · 사립" : ""}${h.coedu && h.coedu !== "남여공학" ? " · " + esc(h.coedu) : ""}${h.adv_pct != null ? " · 대학진학 " + h.adv_pct + "%" : ""}</span></span>`).join("")}</div>
