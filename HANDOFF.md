...
- 2026-09-24 (44차): **경기 출퇴근권 22곳 확장**. 단지 6,831 -> 10,912 (경기 4,081).
  사용자 판단: "경기도가 없어서 반응이 별로". 경기 전체가 아니라 **지하철로 서울에 닿는 곳만** 넣었다.
  우리 무기가 출퇴근 시간 계산이라 지하철이 안 닿으면 기능이 무의미하고 배포 시간만 먹는다. `pipeline/gyeonggi_sgg.py`.
  - 역 317 -> **584**, 학교 1,328 -> 1,987, 학원 25,452 -> **50,991**, 학구도 629 -> 2,379, 동 페이지 231 -> 380+.
  - 원본 자료는 대부분 이미 전국 단위였다. 바꾼 것: build_schoolzones `--sido 11,41`(shp 은 원래 전국),
    fetch_subway/future_rail/amenities/nuisance/poi 의 bbox 를 수도권(37.13,126.55,37.85,127.35)으로,
    fetch_neis 는 경기 행정구역명이 **시 단위**라 13개 시로 수집(교육청 **J10**), refresh.py 에 22곳 추가.
  - **버그 1: geocode.py 가 서울 전용이었다.** 코드표에 없는 시군구를 전부 "서울 마포구"로 질의 -> 경기 단지
    수천 개가 엉뚱한 좌표를 받을 뻔했다. 시도코드 앞 2자리로 서울/경기를 붙이고, 카카오 타임아웃 3회 재시도 추가.
  - **버그 2: OSM 의 station=subway 에 수인분당선·경의중앙선 역이 없다.** 서현·수내·영통이 빠져 분당 단지가
    "모란역 95분"으로 나왔다. 노선 관계 그래프 노드(좌표 포함)로 **153개 보강**. 역 30분 초과 1,972 -> 420.
  - **버그 3: 시군구 코드 2개 오류.** 부천은 2016년 구 폐지 후에도 실거래 API 가 **옛 구 코드로만** 응답한다
    (41190 은 24개월 0건 -> 41192/41194/41196). 41461 은 수지구가 아니라 **처인구**(수지구는 41465).
    처인구 데이터가 "수지구" 이름으로 저장되고 있었다.
  - meta 표기는 `area_label()` 로, 백분위 문구는 "서울 상위" -> "수도권 상위" 로.
  - **주의**: 배포 파일이 2만 2천 개가 되어 한 번에 35분+ 걸린다. 매일 05:00 자동 갱신도 그만큼 길어진다.
  - 남은 약점: 광역버스 데이터가 없어 지하철이 먼 경기 단지는 출퇴근 계산이 안 된다(420개).


- 2026-09-24 (45차): **경기 확장 후 다시 최적화**. 첫 화면 10.6MB -> 5.2MB.
  - **학구도를 별도 파일로 분리**(가장 큰 효과). `school_zones.geojson` 은 학군 레이어 ON + 줌 12.5 이상일 때만 받는다.
    경계는 확대해야 보이는 정보라 첫 화면에 5.6MB 를 받을 이유가 없었다. 색은 로드 시점에 이름 순서로 칠한다.
  - 학구도가 전국 자료라 서비스 범위 밖(평택·안성) 폴리곤까지 있었다. 단지 위치 0.05도 격자로 걸러 596개 제외,
    좌표도 4m -> 12m 간격으로 솎음(22만점 축소).
  - 요약본 중복 제거: `by_area.latest_date`, `station.dist` 는 상세(c/<id>.json)가 덮어써서 목록에서 안 쓰였다.
    `terrain` 은 카드가 쓰는 `station_dh` 만 남김.
  - 측정: complexes.json 파싱 367ms, 첫 화면 gzip 1.02MB. v81 배포·확인 완료.
  - 다음 카드(아직 안 함): 화면에 보이는 지역만 불러오는 방식. 구조 변경이 커서 체감 확인 후 판단.
  - docs/홍보글_초안_카페_2026-09-24.md 로 교체(9/23판 삭제). 수도권 기준 숫자 + "쓰면 안 되는 표현" 절 추가.
